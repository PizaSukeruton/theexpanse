...
function showStep3() {
  ...
  leftPanel.innerHTML = `
    <div style="padding: 15px; height: 100%; overflow-y: auto;">
      <h2 style="color: #00ff75; text-shadow: 0 0 10px #00ff75; margin-bottom: 20px; font-size: 14px;">🎁 GIFT EXPERIMENT WIZARD</h2>
    ...
    <div style="padding: 15px; height: 100%; overflow-y: auto;">
      ...
      <div style="background: rgba(0,255,117,0.05); padding: 12px; border-radius: 4px; margin-bottom: 15px; border: 1px solid rgba(0,255,117,0.2);">
        ...
        <div style="color: #888; font-size: 10px; line-height: 1.8;">
          <div>🎁 Gift PAD: P=${evt.giftObject.pleasure}, A=${evt.giftObject.arousal}, D=${evt.giftObject.dominance}</div>
          <div>👤 Giver: <span data-raccoon="true" data-raccoon-type="character" data-raccoon-value="${evt.giver?.id || ''}" data-raccoon-name="${evt.giver?.name || ''}">${evt.giver?.name || 'N/A'}</span></div>
          <div>📦 Gift: <span data-raccoon="true" data-raccoon-type="event" data-raccoon-value="${evt.selectedGift?.id || ''}" data-raccoon-name="${evt.selectedGift?.name || ''}">${evt.selectedGift?.name || 'N/A'}</span></div>
          <div>👤 Receiver: <span data-raccoon="true" data-raccoon-type="character" data-raccoon-value="${evt.receiver?.id || ''}" data-raccoon-name="${evt.receiver?.name || ''}">${evt.receiver?.name || 'N/A'}</span></div>
          <div>📍 Location: <span data-raccoon="true" data-raccoon-type="location" data-raccoon-value="${evt.location?.id || ''}" data-raccoon-name="${evt.location?.name || ''}">${evt.location?.name || 'N/A'}</span></div>
          <div>🌍 Realm: <span data-raccoon="true" data-raccoon-type="realm" data-raccoon-value="${evt.realm || ''}" data-raccoon-name="${evt.realm || ''}">${evt.realm || 'N/A'}</span></div>
        </div>
      </div>
      <button onclick="executeGiftExchange()" style="width: 100%; padding: 12px; margin-bottom: 8px; background: rgba(0,255,117,0.3); border: 2px solid #00ff75; color: #00ff75; font-family: 'Courier New', monospace; font-size: 12px; cursor: pointer; border-radius: 4px; font-weight: bold;">⚡ EXECUTE GIFT EXCHANGE</button>
...
function showStep7() {
  ...
  leftPanel.innerHTML = `
    <div style="padding: 15px; height: 100%; overflow-y: auto;">
      ...
      <div style="background: rgba(0,255,117,0.05); padding: 12px; border-radius: 4px; margin-bottom: 15px; border: 1px solid rgba(0,255,117,0.2);">
        <div style="color: #00ff75; font-size: 11px; margin-bottom: 10px; font-weight: bold;">✅ GIFT EXCHANGE EXECUTED</div>
        <div style="color: #888; font-size: 10px; line-height: 1.8;">
      ...
      <button onclick="returnToGodMode()" style="width: 100%; padding: 10px; background: rgba(0,255,117,0.2); border: 2px solid #00ff75; color: #00ff75; font-family: 'Courier New', monospace; font-size: 11px; cursor: pointer; border-radius: 4px; font-weight: bold;">✅ COMPLETE & RETURN TO GOD MODE</button>
    </div>
  `;
