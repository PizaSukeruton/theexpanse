...
window.showInventoryManager = async (id) => {
    ...
    try {
        ...
        workspace.innerHTML = `
            <h3 class="section-header">INVENTORY MANIFEST</h3>
                ${window.renderLTLMAnalytics(data.rows)}
...
// ==========================================
// LTLM LTLM dialogue function loader
window.loadLTLMDialogueOptions = async () => {
    try {
        const res = await fetch("/api/admin/ltlm/dialogue-functions");
        const data = await res.json();
        if (!data.success || !Array.isArray(data.items)) return;

        const select = document.getElementById("ltlm-dialogue-code");
        if (!select) return;
        ...
        data.items.forEach(item => {
            ...
            opt.textContent = item.category_code;
            select.appendChild(opt);
        });

        if (currentValue && data.items.some(i => i.category_code === currentValue)) {
            select.value = currentValue;
        }
    } catch (e) {
        console.error("Failed to load LTLM dialogue functions:", e);
    }
};
...
window.showLTLMTrainingPanel = async () => {
    ...
    setTimeout(() => window.loadLTLMDialogueOptions(), 60);
    ...
    try {
        ...
        panel.innerHTML = `
            ...
            <div style="display:flex; flex-wrap:wrap; gap:10px; margin-top:10px;">
                <select id="ltlm-dialogue-code" class="terminal-input" style="flex:1;"></select>
                <select id="ltlm-speech-code" class="terminal-input" style="flex:1;"></select>
                <select id="ltlm-narrative-code" class="terminal-input" style="flex:1;"></select>
                <input id="ltlm-outcome-intents" class="terminal-input" style="flex:1;" placeholder="Outcome intents CSV (optional)">
            </div>

            <div style="display:flex; flex-wrap:wrap; gap:10px; margin-top:10px;">
                <input id="ltlm-pad-pleasure" class="terminal-input" style="flex:1;" placeholder="PAD Pleasure (-1.0 to +1.0)">
                <input id="ltlm-pad-arousal" class="terminal-input" style="flex:1;" placeholder="PAD Arousal (-1.0 to +1.0)">
                <input id="ltlm-pad-dominance" class="terminal-input" style="flex:1;" placeholder="PAD Dominance (-1.0 to +1.0)">
                <input id="ltlm-created" class="terminal-input" style="flex:1;" placeholder="Created By">
            </div>
            <div class="helper-text" style="margin-top:5px; font-size:11px; opacity:0.8;">
                PAD is a 3-number model of emotional tone: Pleasure, Arousal, Dominance. Each value should be between -1.0 and +1.0. If you leave these blank, the system will treat the line as neutral (0.0, 0.0, 0.0).
            </div>
...
window.buildLTLMTrainingLines = (templates, characterId) => {
    const dialogueCode  = document.getElementById("ltlm-dialogue-code")?.value || "turn_management";
    const speechCode    = document.getElementById("ltlm-speech-code")?.value || "assertive";
    const narrativeRaw  = document.getElementById("ltlm-narrative-code")?.value || "";
    const intentsCsv    = (document.getElementById("ltlm-outcome-intents")?.value || "").trim();

    const padPInput = parseFloat(document.getElementById("ltlm-pad-pleasure")?.value);
    const padAInput = parseFloat(document.getElementById("ltlm-pad-arousal")?.value);
    const padDInput = parseFloat(document.getElementById("ltlm-pad-dominance")?.value);

    const safeP = isNaN(padPInput) ? 0.0 : padPInput;
    const safeA = isNaN(padAInput) ? 0.0 : padAInput;
    const safeD = isNaN(padDInput) ? 0.0 : padDInput;

    const padPStr = safeP.toFixed(1);
    const padAStr = safeA.toFixed(1);
    const padDStr = safeD.toFixed(1);

    const narrativeField = narrativeRaw === "" ? "NULL" : narrativeRaw;

    return templates.map(utterance =>
        `${utterance}|${dialogueCode}|${speechCode}|${narrativeField}|${intentsCsv}|${padPStr}|${padAStr}|${padDStr}`
    );
};
...
window.submitLTLM = async (characterId) => {
    ...
    const payload = {
        ...
        created_by: document.getElementById("ltlm-created").value || "cms_ltlm",
    		lines: buildLTLMTrainingLines(templates, characterId)
    };
...
// ==========================================
// LTLM ANALYTICS SUMMARY HELPER
// ==========================================
window.renderLTLMAnalytics = (rows) => {
    ...
    const padCounts = { high: 0, neutral: 0, low: 0 };
    const scoreCounts = { "1": 0, "0": 0, "-1": 0 };
    ...
    for (const r of rows) {
        ...
        // PAD dominance
        if (r.pad_dominance > 0.3) padCounts.high++;
        else if (r.pad_dominance < -0.3) padCounts.low++;
        else padCounts.neutral++;

        // Appropriateness score
        scoreCounts[r.appropriateness_score] =
            (scoreCounts[r.appropriateness_score] || 0) + 1;
    }

    const catList = Object.entries(categoryCounts)
        .map(([k, v]) => `<div>${k}: <b>${v}</b></div>`)
        .join("");

    return `
        <div class="section-header" style="margin-top:15px;">DATA SUMMARY</div>
        <div style="display:flex; gap:40px; margin-bottom:10px;">
            <div>
                <div><b>Total Anchors:</b> ${total}</div>
                <div class="mini-divider"></div>
                <div><b>PAD High:</b> ${padCounts.high}</div>
                <div><b>PAD Neutral:</b> ${padCounts.neutral}</div>
                <div><b>PAD Low:</b> ${padCounts.low}</div>
            </div>
            <div>
                <div><b>Score +1:</b> ${scoreCounts["1"] || 0}</div>
                <div><b>Score 0:</b> ${scoreCounts["0"] || 0}</div>
                <div><b>Score -1:</b> ${scoreCounts["-1"] || 0}</div>
            </div>
            <div>
                <b>Categories:</b>
