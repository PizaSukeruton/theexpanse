...
class KnowledgeAcquisitionEngine {
    constructor() {
        ...
        this.stopWords = new Set([
            ...
            'tell', 'telling', 'told', 'ask', 'asking', 'asked', 'else', 'say', 'saying', 'said', 'get', 'getting', 'got'
        ]);
    ...
    calculateRelevanceScore(item, keywords) {
        ...
        // BOOST: Exact concept match gets massive priority
        for (const keyword of keywords) {
            if (concept === keyword) {
                score += 200; // Exact concept match dominates
                matchedKeywords++;
            } else if (concept.includes(keyword) || keyword.includes(concept)) {
                score += 100; // Partial concept match
                matchedKeywords++;
            }
        }
        ...
        for (const keyword of keywords) {
            ...
            if (contentMatches > 0) {
                matchedKeywords++;
                score += Math.min(25, contentMatches * 15);
            }
        ...
        // Precision bonus: reward matching ALL keywords
        const coverageRatio = matchedKeywords / keywords.length;
        if (coverageRatio === 1.0) {
            score += 50; // All keywords matched - strong bonus
        } else if (coverageRatio >= 0.75) {
            score += 25; // Most keywords matched
        } else if (coverageRatio >= 0.5) {
            score += 10; // Half matched
        }
        ...
        if (matchedKeywords >= 2) {
            ...
            if (keywordPositions.length >= 2) {
                const span = keywordPositions[keywordPositions.length - 1] - keywordPositions[0];
                if (span < 50) score += 20;
                else if (span < 100) score += 10;
            }
        }
    ...
    async searchByConceptThreeTier(query, characterId) {
        ...
        // Try each word directly against the concept field in knowledge_items
        const conceptMatches = new Map();
        
        for (const word of words) {
            try {
                const result = await pool.query(`
                    SELECT DISTINCT ki.concept
                    FROM knowledge_items ki
                    WHERE ki.concept ILIKE $1
                `, ['%' + word + '%']);
                
                for (const row of result.rows) {
                    const concept = row.concept;
                    conceptMatches.set(concept, (conceptMatches.get(concept) || 0) + 1);
                }
            } catch (err) {
                console.error('[KnowledgeAcquisitionEngine] Concept search error:', err.message);
            }
        }
        
        if (conceptMatches.size === 0) {
            return null;
        }
        ...
        for (const [concept, matchCount] of conceptMatches) {
            console.log(`[KnowledgeAcquisitionEngine] Concept "${concept}" matched ${matchCount} keywords`);
        ...
        console.log(`[KnowledgeAcquisitionEngine] Best concept match: "${bestConcept}" (score: ${bestScore}/${words.length})`);
        return bestConcept;
    }
    ...
    async retrieveRelevantKnowledge(characterId, conversationContext, limit = 5) {
        try {
            ...
            // PRIORITY: Try three-tier concept matching first
            const matchedConcept = await this.searchByConceptThreeTier(conversationContext, characterId);
            
            if (matchedConcept) {
                console.log(`[KnowledgeAcquisitionEngine] Final matchedConcept:`, matchedConcept);
                console.log(`[KnowledgeAcquisitionEngine] Using matched concept: ${matchedConcept}`);
                ...
                if (directResult.rows.length > 0) {
                    console.log(`[KnowledgeAcquisitionEngine] Retrieved concept:`, directResult.rows[0].concept, `ID:`, directResult.rows[0].knowledge_id);
                    console.log(`[KnowledgeAcquisitionEngine] Found exact concept match!`);
                    await this.persistLearningEvent(characterId, directResult.rows);
