...
import { lookupExact, lookupInSentence  } from './termIndex';
...
class TmIntentMatcher {
  ...
  static canonicalizeEntity(input, type) {
    ...
    if (type === "role") {
      ...
      import dataSource from './csvDataSource.js';
  ...
  async matchIntent(content, options = {}, member = {}) {
    ...
    if (intent && intent.intent_type && intent.confidence > 0) {
      try {
        import tmAiEngine from './tmAiEngine.js';
...
export default new; TmIntentMatcher();
