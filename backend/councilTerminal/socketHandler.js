...
async function handleOnboardingFlow(socket) {
  try {
    // GUARD: Only run onboarding flow once per socket
    if (socket._onboardingHandled) {
      console.log('[Onboarding] Already handled for this socket, skipping');
      return;
    }
    ...
    switch (state.current_state) {
      case 'new':
        ...
        // Start 15-second timer for proactive omiyage
        socket._omiyageTimer = setTimeout(async () => {
          try {
            ...
            if (currentState && currentState.current_state === 'awaiting_ready') {
              console.log('[Onboarding] 15s timeout - Claude proactively initiating omiyage');
              await onboardingOrchestrator.transitionTo(socket.userId, 'omiyage_offered', {}, 'proactive_timeout');
              const result = await omiyageService.checkAndInitiateOmiyage(socket.userId);
              if (result && result.type !== 'resume_resolved') {
                socket.emit('omiyage:offer', { choiceId: result.choiceId, offerCount: result.offerCount, narrative: result.narrative, giverName: 'Claude The Tanuki' });
              }
            }
          } catch (err) { console.error('[Onboarding] Proactive omiyage error:', err); }
        }, 15000);
        break;
    ...
    socket.on("terminal-command", async (data) => {
      try {
        ...
        if (state && state.current_state === 'awaiting_ready') {
          // ANY input in awaiting_ready triggers omiyage
          if (socket._omiyageTimer) clearTimeout(socket._omiyageTimer);
          console.log('[Onboarding] User input detected, advancing to omiyage');
      ...
      response.source = response.source || "UNSPECIFIED";

        socket.context = response.context || session.context;

        console.log("[DEBUG] ClaudeBrain response:", response);
      // SAFETY GUARD: prevent MechanicalBrain output from reaching users
      if (response?.source === "MechanicalBrain") {
        console.error("[SAFETY] MechanicalBrain attempted user-facing output. Blocking.", response);
