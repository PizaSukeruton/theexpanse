...
class ClaudeBrain {
  ...
  async processQuery(command, session, user) {
    ...
    try {
      ...
      // ==========================================
      // STEP 2.75: EMOTIONAL RESPONSE GATE
      // ==========================================
      if (userPad && (Math.abs(userPad.p) > 0.15 || userPad.a > 0.4)) {
        let emotionalParams = null;
        let responseType = null;

        // Strong negative emotion - empathy/comfort
        if (userPad.p < -0.15) {
          ...
          console.log('[ClaudeBrain] Detected negative emotion (P=' + userPad.p.toFixed(2) + ') - routing to empathy');
        }
        // High arousal distress - calm/contain
        else if (userPad.p < 0 && userPad.a > 0.4) {
          responseType = 'contain_distress';
          emotionalParams = {
            speechActCode: 'expressive.comfort',
            dialogueFunctionCode: 'expressive.comfort',
            outcomeIntentCode: 'emotional_outcomes.contain_affect'
          };
          console.log('[ClaudeBrain] Detected high-arousal distress (P=' + userPad.p.toFixed(2) + ', A=' + userPad.a.toFixed(2) + ') - routing to contain');
        }
        // Strong positive emotion - celebrate/share joy
        else if (userPad.p > 0.35) {
          ...
          console.log('[ClaudeBrain] Detected positive emotion (P=' + userPad.p.toFixed(2) + ') - routing to celebrate');
        }
        // High arousal (excitement/intensity) - acknowledge
        else if (userPad.a > 0.4) {
          responseType = 'acknowledge_intensity';
          emotionalParams = {
            speechActCode: 'expressive.encourage',
            dialogueFunctionCode: 'expressive.encourage',
            outcomeIntentCode: 'emotional_outcomes.validate_experience'
          };
          console.log('[ClaudeBrain] Detected high arousal (A=' + userPad.a.toFixed(2) + ') - acknowledging intensity');
        }
        ...
        if (emotionalParams) {
          try {
            const emotionalUtterance = await selectLtlmUtteranceForBeat({
              ...
              targetPad: { pleasure: userPad.p, arousal: userPad.a, dominance: userPad.d }
            });
