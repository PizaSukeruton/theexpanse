    ...
    socket.on('terminal-command', async (data) => {
      ...
      try {
        ...
        if (response.entityUsed) {
          ...
          if (!socket.context.entityHistory) socket.context.entityHistory = [];
          
          socket.context.entityHistory.push({
            entity: response.entityUsed,
            type: response.entityType,
            query: response.queryType,
            timestamp: Date.now()
          });
          if (socket.context.entityHistory.length > 5) {
            socket.context.entityHistory.shift();
          }
...
Context Stack: ${session.context.entityHistory ? session.context.entityHistory.length : 0} items`
    };
  }
  ...
  // Strip Tanuki Mode prefix for clean intent detection
  let cleanCommand = command;
  const isTanukiMode = command.toLowerCase().includes("tanuki mode");
  if (isTanukiMode) {
    cleanCommand = command.replace(/tanuki\s*mode\s*:?\s*/i, "").trim();
  }

  const intent = await cotwIntentMatcher.matchIntent(cleanCommand, session.context, user);
  console.log("[DEBUG] Intent object RIGHT AFTER matchIntent:", JSON.stringify(intent, null, 2));

  // ═══ TSE KNOWLEDGE ROUTING - CLAUDE ONLY ═══
  const questionIntents = ["WHO","WHAT","WHEN","WHERE","WHY","HOW"];
  const isQuestionIntent = questionIntents.includes(intent.type ? intent.type.toUpperCase() : null);

  if (isTanukiMode && isQuestionIntent) {
    try {
      console.log('[Tanuki] Activating Tanuki Mode with intent:', intent.type);
      ...
      console.log('[Tanuki] Response generated:', tanukiResponse.substring(0, 50) + '...');
      
      return {
        output: tanukiResponse,
        mode: "tanuki",
        intent: intent.type,
        entity: intent.entity,
        confidence: intent.confidence,
        routingStatus: "[Tanuki Mode - MechanicalBrain]",
        entityUsed: intent.entity,
        entityType: intent.type,
        queryType: intent.type
      };
    } catch (tanukiError) {
      console.warn('[Tanuki] Mode failed, falling back to TSE:', tanukiError.message);
    }
  }

  if (isQuestionIntent && !isTanukiMode) {
    try {
      const knowledgeChar = {
        character_id: "#700002",
        character_name: "Claude The Tanuki",
        domain_id: "TANUKI001",
        domain_name: "Tanuki Knowledge"
      };

      const tseEndpoint = process.env.TSE_ENDPOINT || "http://localhost:3000/api/tse/knowledge/response";

      const tseResponse = await fetch(tseEndpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          characterId: knowledgeChar.character_id,
          query: command,
          context: { domain: knowledgeChar.domain_name }
        })
      });

      const tseData = await tseResponse.json();

      if (tseData && tseData.success && tseData.response) {
        const response = {
          output: tseData.response.knowledge || tseData.response,
          routingStatus: "[Tanuki Knowledge - TSE]",
          entityUsed: intent.entity,
          entityType: "Knowledge Entity",
          queryType: intent.type,
          characterId: knowledgeChar.character_id
        };
        return response;
      ...
      console.warn("TSE ROUTING failed, falling back:", tseError.message);
    }
  }

  // --- GOD MODE (LEVEL 11) HANDLER ---
  if (session.accessLevel === 11) {
    let debugOutput = `=== INTENT ANALYSIS ===
Matcher: cotwIntentMatcher
Method: ${intent.matcherMethod ? intent.matcherMethod.toUpperCase() : 'UNKNOWN'}
Type: ${intent.type}
Entity: ${intent.entity}
...
`;
    ...
    let finalQueryType = intent.type;
    let finalUserMessage = `No output generated for ${intent.type} query.`; // Initialize user message
    ...
    if (!intent.godModeSearch) {
      ...
      // Determine Best Match for User Mode
      ...
      if (bestMatch) {
        ...
        try {
          ...
          if (userResult.success) {
            finalUserMessage = userResult.message; // Capture the clean user message
            
            // Capture image URL if SHOW_IMAGE intent
            if (userResult.action === 'show_image' && userResult.data?.image_url) {
                finalImage = userResult.data.image_url;
            }
            
            // Capture data for Memory updates
            finalEntityUsed = bestMatch.entity_name;
            finalEntityType = bestMatch.entity_type;

          } else {
            finalUserMessage = `No information found for: ${intent.entity}`;
          }
        } catch (err) {
          finalUserMessage = `Error generating user response: ${err.message}`;
        }
      } else {
        finalUserMessage = `No matches found for: ${intent.entity}`;
      }
      
      debugOutput += `\n${"=".repeat(60)}\n`;
      
      // *** CRITICAL FIX FOR USER REQUEST ***
      // If the intent is SHOW_IMAGE (which the user just ran), prioritize the clean output 
      // as the primary 'output', and move the full debug log to 'debugLog' to keep it accessible.
      if (intent.type === 'SHOW_IMAGE') {
          return {
            output: finalUserMessage, // Clean output: "Displaying visual for **Piza Sukeruton**"
            entityUsed: finalEntityUsed,
            entityType: finalEntityType,
            queryType: finalQueryType,
            image: finalImage,
            debugLog: debugOutput // Full God Mode log is here
          };
      }
      
      // Default God Mode return (for non-SHOW_IMAGE intents):
      return { 
        output: debugOutput, // Full debug output
        entityUsed: finalEntityUsed,
        entityType: finalEntityType,
        queryType: finalQueryType,
        image: finalImage
    ...
    // Fallback in case there was no search data
...
statusMessage: `INTENT: ${intent.type} (${intent.confidence?.toFixed ? intent.confidence.toFixed(2) : intent.confidence}) – Routed to TSE (domain: pokemon)`,
debug: result.output || "",
queryType: intent.type,
image: tseResponse.image || null,
entityUsed: intent.entity || null,
entityType: result.type || null
};
}
  const imageUrl = result.data?.image_url; // Safely extract image URL
  ...
  // *** NEW: Inject image tag directly into output if available ***
  if (imageUrl) {
    // Inject the image tag into the output string (this assumes the client can render HTML)
    // Adding class 'console-profile-image' for custom styling in the client
    output += `<div class="image-wrapper"><img src="${imageUrl}" alt="${intent.entity}" class="console-profile-image"></div>\n`;
  }
  ...
  const response = {
    output,
    image: imageUrl, // PULLING IMAGE URL FROM THE RESULT DATA
    entityUsed: intent.entity,
