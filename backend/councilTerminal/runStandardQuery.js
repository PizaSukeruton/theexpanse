  ...
  if (shouldQueryKnowledge) {
    ...
    try {
      ...
      if (knowledgeResult && knowledgeResult.found && knowledgeResult.items.length > 0) {
        ...
        const nextIndex = isFollowUpQuery
          ? (lastIndex + 1) % knowledgeResult.items.length
          : 0;

        console.log(`[DEBUG] Knowledge index: lastIndex=${lastIndex}, nextIndex=${nextIndex}, totalItems=${knowledgeResult.items.length}`);

        const primary = knowledgeResult.items[nextIndex];
        ...
        const storytellerResult = await buildStorytellerResponse({
          intentResult,
          emotionalSignal: session?.context?.emotionalSignal || null,
          knowledgeText: responseText,
          tone: 'neutral',
          formality: 'casual'
        });

        console.log("[DEBUG] Storyteller wrapper applied:", storytellerResult.storytellerMeta);

        return {
          output: storytellerResult.output,
          success: true,
          source: "knowledge",
