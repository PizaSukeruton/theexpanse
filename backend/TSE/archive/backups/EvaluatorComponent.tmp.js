...
export default class EvaluatorComponent {
  ...
  async evaluateTaskAttempt({ task, attempt, padSnapshot }) {
    ...
    if (task.taskPhase === "acquisition") {
      ...
      const baseScore = covered ? 3 : 1;
      console.log(`[AcquisitionGate] Applied base score=${baseScore} (attempt length=${attempt.attemptText?.length || 0})`);

      let finalScore = baseScore;
      if (task.characterId) {
        try {
          finalScore = await this._applyTraitWeighting(task.characterId, baseScore);
          console.log(`[TraitWeight-acq] character=${task.characterId} base=${baseScore} → final=${finalScore}`);
        } catch (err) {
          console.error(`[TraitWeightError-acq] ${err.message}`);
        }
      }

      return { score: finalScore, phase: "acquisition", reason: covered ? "sufficient_length" : "too_short" };
    }
  ...
  // Full bidirectional clamp (defensive)
  ...
  _scoreRecall(studentAnswer, expected, task) {
    if (!studentAnswer || studentAnswer.length < 10) return 1;
    if (studentAnswer.includes("remember") || studentAnswer.length > 50) return 4;
    return 3;
  }
