...
import { TASK_CATEGORIES, VALID_CATEGORIES } from "./constants/TaskCategories.js";
export default class TeacherComponent {
  ...
  async teach(characterId, query, context) {
    ...
    let difficulty = context?.difficultyLevel ?? baseDifficulty;
    difficulty = await this._applyAdaptiveDifficulty(characterId, difficulty);

    let task = null;

//     // Phase-aware task gating
//     if (context?.taskPhase === "acquisition") {
//       task = await this._generateAcquisitionTask(characterId, difficulty, context);
//       return task;
//     }


    // NEW: Respect explicit type from TSELoopManager (highest priority)
    ...
    if (requestedType) {
      switch (requestedType) {
        ...
        case 'retrieval':
          ...
          if (dueItems && dueItems.length > 0) {
            ...
            task = {
              ...
              taskType: 'recall',
              taskCategory: TASK_CATEGORIES.ACQUISITION,
              input: `Recall: ${dueItem.content.split("\n")[0]}`,
        ...
        default:
          console.warn(`[TeacherComponent] Unknown requested task type: ${requestedType}`);
          break;
      }
      if (task) return task;
    }

    // COMMUNICATION TASK SELECTOR - Always use communication_quality for "communication" queries
    ...
    if (context?.domainId) {
      ...
      // Stage 9A.3: FINAL authoritative taskType → taskCategory mapping
    ...
    return task;
      ...
      // Stage 9A FINAL: authoritative taskType → taskCategory mapping
    ...
    try {
      ...
      if (context?.domainId) {
        ...
        if (domainItem && domainItem.content) {
          input = `Explain: ${domainItem.content.split("\n")[0]}`;
        }
      }
      ...
      return {
        ...
        taskCategory: TASK_CATEGORIES.COMMUNICATION_QUALITY,
  ...
  async _generateDomainTask(characterId, domainId, difficulty, context) {
    try {
      ...
      const input = `Learn: ${knowledgeItem.content.split("\n")[0]}`;
      ...
      return {
        taskId,
        taskType,
        taskCategory: TASK_CATEGORIES.REWRITE,
  ...
  async _teachLoreFact(characterId, query, difficulty, context) {
    try {
      ...
      if (typeof fact.content === 'string') {
        try {
          content = JSON.parse(fact.content);
        } catch (parseError) {
          console.warn("[TeacherComponent _teachLoreFact] Failed to parse content as JSON:", parseError.message, "Falling back to raw text.");
  ...
  async _generateCommunicationQualityTask(characterId, difficulty, context) {
    return {
      ...
      taskType: "communication_quality",
        taskCategory: TASK_CATEGORIES.COMMUNICATION_QUALITY,
      input: "Explain the concept clearly and warmly, focusing on understanding rather than brevity.",
      difficulty,
      characterId,
      metadata: {
        teacherNode: this.nodeId,
