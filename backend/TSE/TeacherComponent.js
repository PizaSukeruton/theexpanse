...
const LOCKED_DOMAIN_ID = "#AE0008"; // Phase 5 curriculum lock: Hero’s Journey
...
export default class TeacherComponent {
  ...
  async teach(characterId, query, context = {}) {
    ...
    // Domain-aware fallback: try domain task before generic
    if (context?.domainId) {
      const domainFallback = await this._generateDomainTask(characterId, context.domainId, difficulty, context);
      if (domainFallback) {
        console.log(`[Teacher] Domain fallback task - category=${domainFallback.taskCategory}`);
        return domainFallback;
      }
    }

    // HARD STOP: generic fallback forbidden (Phase 5)
    console.error("[TeacherComponent] HARD ERROR: Generic fallback blocked", {
      fallback_used: true,
      fallback_source: "domain_only"
    });
    throw new Error("Generic fallback is forbidden in Phase 5");
  }
