...
class KnowledgeResponseEngine {
    ...
    async shapeKnowledgeDelivery(knowledge, learningProfile, knowledgeNeeds, cognitiveState, query = "", characterId = null) {
        ...
        // Fetch PAD state for emotional modulation
        let padState = null;
        if (this.pool && characterId) {
            try {
                ...
                if (padResult.rows.length > 0) {
                    padState = padResult.rows[0];
                    console.log(`[KnowledgeResponseEngine] Using PAD state for ${characterId}:`, padState);
                }
            } catch (error) {
                console.error('[KnowledgeResponseEngine] Error fetching PAD state:', error.message);
            }
        }
        
        const naturalContent = await this.nlg.generate(knowledge, learningProfile, knowledgeNeeds, query, padState);
        
        return { content: naturalContent, style, traitInfluences };
    }
