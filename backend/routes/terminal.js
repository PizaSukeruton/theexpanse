...
router.post('/query', async (req, res) => {
  ...
  const isTanukiMode = original.toLowerCase().includes("tanuki mode");
  if (isTanukiMode) {
    cleanQuestion = original.replace(/tanuki\s*mode\s*:?\s*/i, "").trim();
  }
  const normalized = normalizeCouncilQuery(cleanQuestion);
  ...
  const response = await terminal.processQuery(cleanQuestion, userId);
