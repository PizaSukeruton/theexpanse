...
class KnowledgeAcquisitionEngine {
    ...
    async searchByConceptThreeTier(query, characterId) {
        ...
        for (const word of words) {
            try {
                // Direct SQL search on concept field (case-insensitive)
                const result = await pool.query(`
                    SELECT DISTINCT ki.concept
                    FROM knowledge_items ki
                    JOIN character_knowledge_state cks ON ki.knowledge_id = cks.knowledge_id
                    WHERE cks.character_id = $1 
                      AND ki.concept ILIKE $2
                    LIMIT 1
                `, [characterId, word + '%']);
                
                if (result.rows.length > 0) {
                    const matchedConcept = result.rows[0].concept;
                    console.log(`[KnowledgeAcquisitionEngine] Direct concept match: ${word} → ${matchedConcept}`);
