...
async function getTestUserIds() {
  console.log('🔍 Looking up test user IDs...');
  for (const username of TEST_USERNAMES) {
    ...
    if (result.rows.length > 0) {
      ...
      console.log(`  ✓ ${username}: ${result.rows[0].user_id}`);
    } else {
      console.warn(`  ⚠️  ${username}: Not found in database`);
    }
  }
...
// ============================================================================
// TEST 1: First Login → awaiting_ready
// ============================================================================
async function test1_firstLogin() {
  ...
  const audit = await pool.query(
    "SELECT COUNT(*) as count FROM user_onboarding_audit WHERE user_id = $1 AND reason = 'welcome_flow_atomic'",
    [testUserId]
  );
  
  console.log("[DEBUG] Audit count result:", audit.rows[0]);
  console.log("[DEBUG] Audit count result:", audit.rows[0]);
  await assert(parseInt(audit.rows[0].count) === 1, 'Single audit entry for atomic transition');
}
...
// ============================================================================
// TEST 3: Double "Yes" → One Success, One No-Op
// ============================================================================
...
async function test5_adminOverride() {
  ...
  const testUserId = testUserIds[2] || testUserIds[0];
  await cleanupTestUser(testUserId);
  
  // Setup: User at omiyage_offered (one step before completion)
  await onboardingOrchestrator.initializeUser(testUserId);
  await onboardingOrchestrator.advanceToAwaitingReadyAfterWelcome(testUserId, '#BEAT05');
  await onboardingOrchestrator.transitionTo(testUserId, 'omiyage_offered', {}, 'test_setup');
  
  // Admin completes onboarding for stuck user
  await onboardingOrchestrator.transitionTo(
    testUserId,
    'onboarded',
    { admin_override: true },
  ...
  await assert(state.current_state === 'onboarded', 'Admin can force complete stuck user');
  ...
  await assert(
    audit.rows.length > 0 && audit.rows[0].reason.includes('admin_force'),
    'Audit trail contains admin override reason'
  );
}
async function runAllTests() {
  console.log('🧪 Starting FSM Integration Tests...\n');
  ...
  try {
    ...
    if (testsFailed === 0) {
      console.log('\n🎉 All tests passed! FSM integration is working correctly.\n');
    } else {
