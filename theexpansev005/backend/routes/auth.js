...
import { upsertDossierForUser } from '../../services/cotwDossierService.js';
...
router.post('/login', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT user_id, username, access_level, password_hash FROM users WHERE username = $1',
      [username]
    );
    ...
    // Update or create COTW dossier for this user
    try {
      await upsertDossierForUser(user.user_id);
    } catch (e) {
      console.error('COTW dossier upsert on login failed:', e);
    }
