...
router.get('/ltlm/dialogue-functions', verifyAdminAuth, async (req, res) => {
  try {
    const result = await pool.query(
      ...
      SELECT
        ...
        description,
        is_active
      FROM dialogue_function_categories
    ...
    console.error('Error fetching LTLM dialogue functions:', error);
    return res.status(500).json({
      success: false,
      message: 'Failed to load LTLM dialogue functions',
      error: error.message
    });
  }
});
