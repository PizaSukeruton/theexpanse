...
class OnboardingOrchestrator {
  ...
  async advanceToAwaitingReadyAfterWelcome(userId, welcomeBeatId = null) {
    ...
    try {
      ...
      await this.executeBeforeHooks(userId, fromState, 'awaiting_ready', stateData);
      ...
      console.log('[OnboardingOrchestrator] Atomic transition:', {
        userId,
        transition: `${fromState} → awaiting_ready`,
        version: `v${fromVersion} → v${toVersion}`,
        reason: 'welcome_flow_atomic'
      });

      await this.executeAfterHooks(userId, fromState, 'awaiting_ready', newState);
      ...
      console.error('[OnboardingOrchestrator] Atomic transition failed:', {
        userId,
