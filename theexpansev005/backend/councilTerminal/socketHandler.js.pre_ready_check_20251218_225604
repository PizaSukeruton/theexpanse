...
export default function initializeWebSocket(httpServer, sessionMiddleware) {
  ...
  terminalIo.use(async (socket, next) => {
    ...
    // Robust fix: fetch ownedCharacterId from DB if missing in session
    if (sess.ownedCharacterId) {
      ...
      try {
        ...
        if (result.rows.length > 0 && result.rows[0].owned_character_id) {
          ...
          // Update session for future connections
  ...
  terminalIo.on("connection", (socket) => {
    ...
    (async () => {
      try {
        ...
        // Mark user as seen (regardless of welcome beat)
        seenWelcomeUsers.add(socket.userId);

        // Trigger Omiyage check (regardless of welcome beat)
