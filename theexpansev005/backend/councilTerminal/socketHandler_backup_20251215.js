...
async function maybeEmitOmiyage(socket) {
  const existing = await pool.query(`
    SELECT 1 FROM omiyage_events WHERE receiver_character_id = $1 LIMIT 1
  `, [socket.userId]);

  if (existing.rows.length > 0) {
    console.log("[OmiyageDebug] Omiyage already exists, skipping for", socket.userId);
    return;
  }

  console.log("[OmiyageDebug] maybeEmitOmiyage called for", socket.userId);

  const TEST_OBJECT_ID = null;
  if (!TEST_OBJECT_ID) {
    console.log("[OmiyageDebug] Skipping gift logic — no object selected yet");
    return;
  }

  await giftEventHandler.handleGiftReceived(
    socket.userId,
    "700002",
    TEST_OBJECT_ID
  );
...
export default function initializeWebSocket(httpServer, sessionMiddleware) {
  ...
  console.log("[OmiyageDebug] post-greeting hook reached for", socket.userId);
    ...
    socket.on("terminal-command", async (data) => {
      try {
        ...
        const modeResult = await modeRouter.detectMode(command, session.access_level, session);
        ...
        if (modeResult.mode === "god") {
          ...
          response = await GodModeDebugger(modeResult.intentResult, user, command, session);
        } else if (modeResult.mode === "tanuki") {
          console.log("[Socket] TANUKI mode. Routing to MechanicalBrain...");
          ...
          const intentResult = modeResult.intentResult;

          const normalResult = await runStandardQuery(
            intentResult,
            session,
            modeResult.cleanCommand
          );

          response = await TanukiModeRouter(
            intentResult,
            user,
            modeResult.cleanCommand,
            session,
        ...
        // STEP 4 CHANGE: Capture storeContext from knowledge response
        if (response?.storeContext) {
          console.log("[Socket] Capturing knowledge context");
          session.context = session.context || {};
          session.context.lastDomains = response.storeContext.lastDomains;
          session.context.lastEntity = response.storeContext.lastEntity;
          session.context.knowledgeIds = response.storeContext.knowledgeIds;
          session.context.lastKnowledgeIndex = response.storeContext.lastKnowledgeIndex;
  ...
  socket.on("omiyage:accept", async (payload) => {
    console.log("[Omiyage] accept received", {
      user: socket.userId,
      payload
    });
  });
  ...
  socket.on("omiyage:decline", async (payload) => {
    console.log("[Omiyage] decline received", {
      user: socket.userId,
      payload
    });
  });
  ...
  socket.on("omiyage:deferral", async (payload) => {
    console.log("[Omiyage] deferral received", {
      user: socket.userId,
      payload
    });
  });
