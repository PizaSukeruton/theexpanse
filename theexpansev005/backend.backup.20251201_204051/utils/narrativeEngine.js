...
async function getStorytellerStep(characterId) {
    if (!validateHexId(characterId)) {
        throw new Error('Invalid character ID format.');
    }

    try {
        const { StorytellerBridge } = await import('../TSE/systems/StorytellerBridge.js');
        const bridge = new StorytellerBridge();
        return await bridge.getNextStoryBeat(characterId);
    } catch (error) {
        console.error(`[NarrativeEngine] Error getting storyteller step for ${characterId}:`, error.message);
        throw error;
    }
}
