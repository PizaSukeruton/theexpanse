  ...
  leftPanel.innerHTML = `
    <div class="admin-panel" style="padding: 15px; height: 100%; overflow-y: auto;">
      <h2 style="color: #00ff00; text-shadow: 0 0 10px #00ff00; margin-bottom: 20px; font-size: 16px; text-align: center;">
        PIZA SUKERUTON MULTIVERSE ADMIN
      </h2>
...
window.createEditModal = function(character) {
  ...
  modal.innerHTML = `
    <div class="modal-content">
      <h2>EDIT CHARACTER: ${character.character_id}</h2>
      <form id="edit-character-form">
        <label>Name:</label>
        <input type="text" id="edit-name" value="${character.character_name}">
        
        <label>Category:</label>
        <input type="text" id="edit-category" value="${character.category || ''}">
        
        <label>Description:</label>
        <textarea id="edit-description" rows="4">${character.description || ''}</textarea>
        
        <div class="modal-buttons">
          <button type="submit">SAVE CHANGES</button>
          <button type="button" onclick="closeEditModal()">CANCEL</button>
          <button type="button" class="delete-btn" onclick="deleteCharacter('${character.character_id}')">DELETE</button>
        </div>
      </form>
    </div>
  `;
...
// Add return to admin menu button at the top
const adminPanel = document.querySelector('.admin-panel');
if (adminPanel && !document.getElementById('return-to-admin-btn')) {
  const returnBtn = document.createElement('button');
  returnBtn.id = 'return-to-admin-btn';
  returnBtn.textContent = 'RETURN TO ADMIN MENU';
  returnBtn.style.cssText = 'width: 100%; padding: 10px; margin-bottom: 10px; background: rgba(0,255,117,0.1); border: 2px solid #00ff75; color: #00ff75; font-family: "Courier New", monospace; font-size: 12px; cursor: pointer; border-radius: 8px; font-weight: bold;';
  returnBtn.onclick = () => {
    if (typeof initAdminPanel === 'function') {
      initAdminPanel();
    }
  };
  adminPanel.insertBefore(returnBtn, adminPanel.firstChild);
}
