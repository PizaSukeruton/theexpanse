...
// Removed: import { image_retrieval } from '../api.js'; 
...
class CotwQueryEngine {
  ...
  async processQuery(intentResult, user) {
    ...
    // Fallback for general image search that didn't match a specific entity
    if (type === 'SHOW_IMAGE') {
        return await this.handleShowImage({ entity_name: entity, entity_type: 'SEARCH' }, realm);
    }
  ...
  async handleIntentType(type, entityData, realm) {
    switch (type) {
      ...
      case 'EDIT_PROFILE': return this.handleEditProfile(entityData, realm); // New intent handler
  ...
  async handleShowImage(entityData, realm) {
    ...
    // Fallback: If no internal image, inform the user (We removed the dependency on the external tool for now)
    return {
      success: true,
      message: `Image request processed for **${entityName}**, but no internal image URL found in the database.`,
      data: entityData,
      realm,
      image: null
    };
  ...
  /**
   * EDIT_PROFILE handler - Explicitly for opening the editor (new for security)
   */
  async handleEditProfile(entityData, realm) {
    const { entity_name } = entityData;
    
    // In a real application, this handler would only return the necessary data 
    // and a specific action flag that the client uses to switch to the full editor UI.
    return {
      success: true,
