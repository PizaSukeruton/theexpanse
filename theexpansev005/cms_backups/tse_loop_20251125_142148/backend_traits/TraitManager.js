...
class TraitManager {
    ...
    async getTraitVector(characterId) { // RENAMED from getCharacterTraitScores
        try {
            ...
            // NOTE: The column name in character_trait_scores is 'percentile', not 'percentile_score'.
            const query = `
            ...
            res.rows.forEach(row => {
                traitScores[row.trait_hex_color] = row.percentile_score; // Use 'percentile' column
            });
            return traitScores;
        } catch (error) {
    ...
    async updateTrait(characterId, traitHexId, delta) {
        try {
            ...
            const currentScoreQuery = `SELECT percentile FROM character_trait_scores WHERE character_hex_id = $1 AND trait_hex_color = $2;`;
