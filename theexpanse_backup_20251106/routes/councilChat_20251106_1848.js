...
router.post('/chat', async (req, res) => {
  try {
    const userId = req.session?.userId || "debug-test";
    ...
    else if (lower.startsWith('show arc #')) {
    ...
    else if (lower.endsWith(' arc?') || lower.endsWith(' arc.')) {
    ...
    else if (lower.startsWith('show details for event #')) {
    ...
    else if (lower.startsWith('who was involved in event #')) {
      const eventId = extractId(lower, /#([0-9a-z]{6,7})/i);
      if (!eventId) return res.json({ text: 'Invalid event id.', type: 'error' });
    ...
    // New: "who was sighted at <location>?"
    else if (lower.startsWith('who was sighted at ')) {
      const loc = lower.replace(/^who was sighted at\s+/, '').replace(/\?+$/, '').trim();
      const like = `%${loc}%`;
      const r = await pool.query(
        `SELECT involved_characters FROM multiverse_events
         WHERE location ILIKE $1 AND LOWER(event_type) = 'sighting'
         ORDER BY timestamp DESC LIMIT 200`, [like]
      );
      // Flatten unique character names
      const set = new Set();
      r.rows.forEach(row => {
        if (Array.isArray(row.involved_characters)) {
          row.involved_characters.forEach(ch => set.add(ch));
        }
      });
    ...
    else if (lower.startsWith('what events happened at ')) {
    ...
    else if (lower.startsWith('list events with event_type ')) {
      const { limit, offset } = extractLimitOffset(lower);
      const sort = extractSort(lower);
    ...
    else if (lower.startsWith('list events since ')) {
      const { limit, offset } = extractLimitOffset(lower);
      const sort = extractSort(lower);
    ...
    else if (lower.startsWith('find events near ')) {
      const { limit, offset } = extractLimitOffset(lower);
    ...
    else if (lower.startsWith('list events in realm ')) {
      const { limit, offset } = extractLimitOffset(lower);
      const sort = extractSort(lower);
