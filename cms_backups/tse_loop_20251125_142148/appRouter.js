...
import '../godmode-menu.js';
import './modules/chatDisplay.js';
import './modules/menuBuilderWizard.js';
import './modules/pizzaMenu.js';
...
const scenes = {
  ...
  terminal: (accessLevel) => {
    // Dynamic scene based on access level
    const menuModule = accessLevel === 11 ? 'admin/godMode' : 'admin/pizzaMenu';
    return [
      { windowId: 'left-panel', moduleId: menuModule },
...
function extractContext(sceneName) {
  ...
  if (sceneName === 'terminal') {
    const accessLevel = params.get('access_level');
    if (accessLevel) {
      context.accessLevel = parseInt(accessLevel);
      console.log('[AppRouter] Extracted access_level:', context.accessLevel);
    }
  }
...
function loadScene(sceneName) {
  ...
  // If scene is a function (like terminal), call it with access level
  if (typeof scene === 'function') {
    const accessLevel = sceneContext.accessLevel || 1; // Default to level 1
    scene = scene(accessLevel);
    console.log('[AppRouter] Dynamic scene for access level:', accessLevel);
  }
