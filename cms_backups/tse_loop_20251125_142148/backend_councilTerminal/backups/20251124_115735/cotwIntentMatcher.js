...
class CotwIntentMatcher {
  ...
  async matchIntent(query, context = null, user = null, realmOverride = null) {
    ...
    // GOD MODE: Use all-tiers search for Level 11
    ...
    if (context && context.lastEntity) {
      ...
      if (pronounPatterns.test(normalized)) {
        ...
        if (useGodMode) {
          const allTiersResult = await searchEntityAllTiers(context.lastEntity, realm_hex_id);
          return {
            type: intentWord,
    ...
    // Handle confirmation responses (yes/no) when context exists
